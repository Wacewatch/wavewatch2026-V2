...
export default function MovieDetailPage() {
  ...
  useEffect(() => {
    ...
    // Fetch logo
    API.get(`/api/tmdb/movie/${id}/images`)
      .then(({ data }) => {
      ...
      <div className="container mx-auto px-4 -mt-16 md:-mt-32 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 md:gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            {/* Like/Dislike */}
            <LikeDislike contentId={parseInt(id)} contentType="movie" />
            ...
            {similar.length > 0 && (
              <div className="space-y-4">
                ...
                <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide">
                  {similar.map(s => <div key={s.id} className="flex-shrink-0 w-36"><ContentCard item={s} type="movie" /></div>)}
                </div>
              </div>
            )}
          </div>
